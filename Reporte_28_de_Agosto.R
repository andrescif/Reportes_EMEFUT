...
#Cargar datos
Avances.hasta.el.viernes.28.de.agosto <- 
  read.csv("~/R Projects/Reportes_EMEFUT/Datos/Avances hasta el viernes 28 de agosto.csv")

#Variable
datos <- Avances.hasta.el.viernes.28.de.agosto
rm(Avances.hasta.el.viernes.28.de.agosto)
...
#Filtrar las fechas
datos_agosto<- datos %>%
  filter(datos$`Fecha de contacto`>"2020-08-13" &
           datos$`Fecha de contacto`<"2020-08-29")
#Resumen del reporte
...
write.csv(Informe_Quincena_Agosto,
          file = "Tabla Segunda Quincena Agosto")
rm(list = ls())
